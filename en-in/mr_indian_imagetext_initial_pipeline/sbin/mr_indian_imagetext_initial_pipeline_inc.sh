...
#    HadoopFileSysMakeDir ${HADOOP_IMAGETEXT_PIPELINE_ANNOTATOR_DIR}
    ...
    # the last process time for inc process
    IMAGETEXT_INC_TIMESTAMPFILE=${IMAGETEXT_PIPELINE_STATUS_DIR}/mr_${COMMAND}_inc.timestamp
    file_last_time=
    file_last_time=`cat $IMAGETEXT_INC_TIMESTAMPFILE`
    ...
    cur_file_last_time=`hadoop fs -ls $input_hadoop_inc_file | awk '{print $6" "$7}'`

    cur_tag=$input_hadoop_inc_file" "$cur_file_last_time
    LoggerInfo $cur_tag
    LoggerInfo $file_last_time
    if [[ $file_last_time == $cur_tag ]]
    then
        LoggerError "Inc file dosenot changed !!!"
        return 1
    fi
...
#        MrImageTextPipelineAlert ${IMAGETEXT_PIPELINE_LOGFILE}
    ...
    echo $cur_tag > $IMAGETEXT_INC_TIMESTAMPFILE
    return 0;
}
